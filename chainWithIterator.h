...
class chain : public linearList<T> 
{
   public:
      ...
      // split member function problem #22
...
{
	// splits calling chain so all the odd elements of it are in a and the rest are in b
	int j = 0, k = 0, size = this->listSize;
	for(int i = 0; i < size; i++)
	{
		if(i%2 == 0) // puts all the even elements into b
		{
			b.insert(j,this->get(0));
			this->erase(0);
			j++;
		}
		else if(i%2 == 1) // puts all the odd elements in a
		{
			a.insert(k,this->get(0));
			this->erase(0);
			k++;
		}
	}
